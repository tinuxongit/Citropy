    ...
    <section className="computer-pane scroll" aria-label={t("Computer use")}>
      ...
      {!running ? (
        <div className="computer-welcome">
          ...
          {state.status !== "starting" && <small>{capabilities?.available ? t("{backend} · One conversation at a time", { backend: capabilities.backend === "wayland-portal" ? t("Wayland · Desktop consent") : t("X11 · Desktop session") }) : capabilities?.reason ?? t("Checking desktop support…")}</small>}
        </div>
      ) : (
        <>
