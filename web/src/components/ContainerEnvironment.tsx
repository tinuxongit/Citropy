...
export function ContainerEnvironment({ onClose, connection }: { onClose: () => void; connection?: SshConnection }) {
  ...
  return <Modal title={t("Container workspace")} icon={<Box size={20} />} description={t("Run providers and shells in a Linux container using Docker.")} busy={busy} onClose={onClose} onSubmit={() => void connect()} footer={<>
    ...
    <button className="btn" data-variant="primary" type="submit" disabled={busy || !path || !name.trim()}>{busy && <PixelLoader size={14} />}{t(id ? "Reconnect" : "Create and connect")}</button>
  </>}>
    ...
    <label className="feature-field">{t("Mounted folder")}<div className="feature-inline"><input readOnly value={path} placeholder={t("Choose a folder")} /><button className="btn" type="button" disabled={busy || Boolean(id)} onClick={() => void desktop?.chooseWorkspaceFolder("local", path).then(value => { if (value) { setPath(value); if (!name) setName(value.split(/[\\/]/).filter(Boolean).at(-1) ?? "Container"); } }).catch(error => setError(error.message))}><FolderOpen size={15} />{t("Browse")}</button></div></label>
    ...
    {busy && <p className="ssh-progress" role="status"><PixelLoader size={14} />{t(state.connections.find(entry => entry.id === id)?.message ?? "Connecting…")}</p>}
